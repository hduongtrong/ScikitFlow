...
class LogisticRegression(BaseEstimator):
    def __init__(self, n_epoch = 2, batch_size = 128, ridge = 0., verbose = 0):
    ...
    def fit(self, X, Y):
        ...
        cross_entropy = -tf.reduce_sum(y * tf.log(p)) / n + \
                self.ridge * tf.reduce_sum(W**2)
...
if __name__ == '__main__':
    ...
    clf = LogisticRegression(n_epoch = 100, ridge = 0.0001)
    clf.fit(X, Y)
